...
sudo ip addr flush dev $INT_LAN
sudo ip addr flush dev $INT_DMZ
sudo ip addr flush dev $INT_WAN
...
table inet filter {
    set ALLOW_L {
        type ipv4_addr;
        flags interval;
        elements = { $ALLOW_LAN, $ALLOW_BRIDGE, $ALLOW_DMZ, $ALLOW_EXTERNAL, $ALLOW_LAN2 }
    }
